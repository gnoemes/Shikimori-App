...
class MainActivity : BaseActivity<MainPresenter, MainView>(), MainView, RouterProvider {
    ...
    override fun onRestoreInstanceState(savedInstanceState: Bundle?) {
        ...
        if (bottomNav != null && savedInstanceState != null) {
            bottomNav.check(savedInstanceState.getInt(AppExtras.ARGUMENT_SELECT_TAB, R.id.tab_favorite))
        }
    }
    ...
    override fun initBottomNavigation() {
        ...
        with(bottomNav) {
            ...
            check(tabs.first().id)
            setOnClickListener { Log.i("DEVE", "id: ${it.id}") }
            setOnCheckChangedListener { oldIdRes, newIdRes ->
                val tab = tabs.firstOrNull { it.id == newIdRes }
                //TODO rewrite lib to support recheck
...
//                } else {
    ...
    private fun createItem(tab: Tab): INavigationBarItem = object : AbsNavigationBarItem() {
        override fun getColorInt(isChecked: Boolean): Int? = if (isChecked) colorAttr(R.attr.colorBottomAccent) else colorAttr(R.attr.colorBottomDark)
        override fun getDrawableIdRes(isChecked: Boolean): Int = tab.icon
        override fun getText(isChecked: Boolean): String? = null
        override fun getIdRes(): Int = tab.id
        override fun getTextColorInt(isChecked: Boolean): Int = if (isChecked) colorAttr(R.attr.colorBottomAccent) else colorAttr(R.attr.colorBottomDark)
    }
